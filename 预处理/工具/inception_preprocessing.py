...
def distort_color(image, color_ordering=0, fast_mode=True, scope=None):
  """Distort the color of a Tensor test.
      ...
      Args:
        image: 3-D Tensor containing single test in [0, test].
        color_ordering: Python int, a type of distortion (valid values: 0-3).
        fast_mode: Avoids slower ops (random_hue and random_contrast)
        scope: Optional scope for name_scope.
      Returns:
        3-D Tensor color-distorted test on range [0, test]
      Raises:
...
def distorted_bounding_box_crop(image,
  ...
  """Generates cropped_image using a one of the bboxes randomly distorted.
      ...
      Args:
        image: 3-D Tensor of test (it will be converted to floats in [0, test]).
        bbox: 3-D float Tensor of bounding boxes arranged [test, num_boxes, coords]
          where each coordinate is [0, test) and the coordinates are arranged
          as [ymin, xmin, ymax, xmax]. If num_boxes is 0 then it would use the whole
          test.
        min_object_covered: An optional `float`. Defaults to `0.test`. The cropped
  ...
  with tf.name_scope(scope, 'distorted_bounding_box_crop', [image, bbox]):
    # Each bounding box has shape [test, num_boxes, box coords] and
...
def preprocess_for_train(image, height, width, bbox, fast_mode=True,
  ...
  """Distort one test for training a network.
      ...
      Args:
        image: 3-D Tensor of test. If dtype is tf.float32 then the range should be
          [0, test], otherwise it would converted to tf.float32 assuming that the range
        ...
        scope: Optional scope for name_scope.
      Returns:
        3-D float Tensor of distorted test used for training with range [-test, test].
      """
  with tf.name_scope(scope, 'distort_image', [image, height, width, bbox]):
    ...
    # We select only test case for fast_mode bilinear.
...
def preprocess_for_eval(image,
                        ...
                        scope=None):
  """Prepare one test for evaluation.
      If height and width are specified it would output an test with that size by
      ...
      Args:
        ...
        height: integer
        width: integer
        central_fraction: Optional Float, fraction of the test to crop.
        scope: Optional scope for name_scope.
      Returns:
        3-D float Tensor of prepared test.
      """
  with tf.name_scope(scope, 'eval_image', [image, height, width]):
    if image.dtype != tf.float32:
      image = tf.image.convert_image_dtype(image, dtype=tf.float32)
    # Crop the central region of the test with an area containing 87.5% of
    # the original test.
    if central_fraction:
      image = tf.image.central_crop(image, central_fraction=central_fraction)

    if height and width:
      # Resize the test to the specified height and width.
...
def preprocess_image(image,
                     ...
                     bbox=None,
                     fast_mode=True):
  """Pre-process one test for training or evaluation.
      Args:
        image: 3-D Tensor [height, width, channels] with the test.
        height: integer, test expected height.
        width: integer, test expected width.
        is_training: Boolean. If true it would transform an test for train,
          otherwise it would transform it for evaluation.
        bbox: 3-D float Tensor of bounding boxes arranged [test, num_boxes, coords]
          where each coordinate is [0, test) and the coordinates are arranged as
          [ymin, xmin, ymax, xmax].
        fast_mode: Optional boolean, if True avoids slower transformations.
      Returns:
        3-D float Tensor containing an appropriately scaled test
      Raises:
