...
def main(_):
  ...
  _ = tf.histogram.summary('weights', W)
  _ = tf.histogram.summary('biases', b)
  _ = tf.histogram.summary('y', y)
  ...
  with tf.name_scope('xent'):
    ...
    _ = tf.scalar.summary('cross entropy', cross_entropy)
  with tf.name_scope('train'):
    train_step = tf.train.GradientDescentOptimizer(
        FLAGS.learning_rate).minimize(cross_entropy)

  with tf.name_scope('python_base'):
  ...
  # Train the model, and feed in python_base data and record summaries every 10 steps
